...
static void maccpy_reverse(uint8_t dst[6], uint8_t src[6]) {
  for (int i = 0; i < 6; i++) dst[i] = src[5 - i];
}

static void mac_log_if_diff_src(struct mgos_bt_addr *src, uint8_t adv_mac[6],
                                struct json_out *out) {
  struct mgos_bt_addr *adv = (void *) adv_mac;
  if (!mgos_bt_addr_cmp(src, adv)) return;
  json_printf(out, " (%s %s)", "adv MAC",
              mgos_bt_addr_to_str(adv, 0, alloca(MGOS_BT_ADDR_STR_LEN)));
}
...
static bool ble_adv_atc_pvvx_decode(struct mgos_bt_gap_scan_result *r,
  ...
  maccpy_reverse(data->mac, adv->mac);
  data->cnt = adv->cnt;
  ...
  json_printf(ok, "%c %.2f%s, %c %.2f%%, %c %u%% (%u %s), # %u, %s %x", 'T',
              ...
              data->flags);
  mac_log_if_diff_src(&r->addr, data->mac, ok);
  return true;
}
...
static bool ble_adv_mi_enc_decode(struct mgos_bt_gap_scan_result *r,
  ...
  maccpy_reverse(data->mac, top->mac);
...
static void ble_adv_log(struct mgos_bt_gap_scan_result *r,
...
void atc_mi_handle(struct mgos_bt_gap_scan_result *r) {
  ...
  if (fmt->decode(r, atc_mi, &data, ok, fail)) {
    if (ok) ble_adv_log(r, fmt, atc_mi, "accepted", ok->u.buf.buf);
